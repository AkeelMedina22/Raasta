...
def plot_graphs(history, string):
    ...
    plt.savefig("experiment/model_"+string)
    plt.show()

# fit and evaluate a model
def evaluate_model(X_train, Y_train, X_test, Y_test):

    verbose, epochs, batch_size = 1, 200, 4
    n_timesteps, n_features, n_outputs = X_train.shape[1], X_train.shape[2], Y_train.shape[1]

    model = tf.keras.Sequential([Dense(32, input_shape=(n_timesteps,n_features)),
    tf.keras.layers.LeakyReLU(),
    Conv1D(filters=16, kernel_size=3, kernel_regularizer=tf.keras.regularizers.l1(0.0005)),
    tf.keras.layers.LeakyReLU(),
    Conv1D(filters=32, kernel_size=3, kernel_regularizer=tf.keras.regularizers.l1(0.0005)),
    tf.keras.layers.LeakyReLU(),
    MaxPooling1D(pool_size=2, strides=2),
    Conv1D(filters=32, kernel_size=5, kernel_regularizer=tf.keras.regularizers.l1(0.0005)),
    tf.keras.layers.LeakyReLU(),
    Conv1D(filters=64, kernel_size=5, kernel_regularizer=tf.keras.regularizers.l1(0.0005)),
    tf.keras.layers.LeakyReLU(),
    MaxPooling1D(pool_size=2, strides=2),
    tf.keras.layers.Dropout(0.4),
    tf.keras.layers.Bidirectional(tf.keras.layers.LSTM(32, kernel_regularizer=tf.keras.regularizers.l1(0.0005))),
    tf.keras.layers.LeakyReLU(),
    tf.keras.layers.Dropout(0.2),
    Dense(n_outputs, activation='sigmoid')])

    model.compile(loss='binary_crossentropy', optimizer=tf.keras.optimizers.SGD(), metrics=[tf.keras.metrics.BinaryAccuracy(), tf.keras.metrics.Precision(), tf.keras.metrics.Recall()])
    ...
    plt.tight_layout()
    plt.legend()
    plt.savefig("experiment/matplotlib_visualization")
    plt.show()
    ...
    def show_confusion_matrix(cm, labels):
        ...
        plt.ylabel('Label')
        plt.savefig("experiment/confusion_matrix")
        plt.show()
    ...
    this_map.save('experiment/folium_visualization.html')
