...
def evaluate_model(X_train, Y_train, X_test, Y_test):
    ...
    for i in range(X_train.shape[0]):
        # print(np.array(X_train[i].flatten().reshape(-1, 1).T).shape)
        # f, t, Sxx = spectrogram(X_train[i].flatten().reshape(-1,1), 10, mode="psd", nperseg=360)
        ...
        mels = librosa.feature.melspectrogram(y=X_train[i].flatten().reshape(-1,1), sr=10.0)
    ...
    for i in range(X_test.shape[0]):
        ...
        mels = librosa.feature.melspectrogram(y=X_test[i].flatten().reshape(-1,1), sr=10.0)
        mels = np.log(mels + 1e-9) # add small number to avoid log(0)
