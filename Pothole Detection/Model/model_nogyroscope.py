...
def evaluate_model(X_train, Y_train, X_test, Y_test):
    ...
    n_timesteps, n_features, n_outputs = X_train.shape[1], X_train.shape[2], Y_train.shape[1]
    model = tf.keras.Sequential([Dense(64, activation='relu', input_shape=(n_timesteps,n_features)),
    # Conv1D(filters=32, kernel_size=3, activation='relu', kernel_regularizer=tf.keras.regularizers.l1(0.0005)),
    Conv1D(filters=64, kernel_size=3, activation='relu', kernel_regularizer=tf.keras.regularizers.l1(0.0005)),
    MaxPooling1D(pool_size=2, strides=2),
    # Conv1D(filters=64, kernel_size=5, activation='relu', kernel_regularizer=tf.keras.regularizers.l1(0.0005)),
    Conv1D(filters=128, kernel_size=5, activation='relu', kernel_regularizer=tf.keras.regularizers.l1(0.0005)),
    MaxPooling1D(pool_size=2, strides=2),
    tf.keras.layers.Dropout(0.4),
    # tf.keras.layers.Bidirectional(tf.keras.layers.LSTM(
    #     64, activation='relu', return_sequences=True, kernel_regularizer=tf.keras.regularizers.l2(0.001))),
    # tf.keras.layers.Bidirectional(tf.keras.layers.LSTM(
    #     64, activation='relu', return_sequences=True, kernel_regularizer=tf.keras.regularizers.l2(0.001))),
    tf.keras.layers.Bidirectional(tf.keras.layers.LSTM(
        300, activation='relu', kernel_regularizer=tf.keras.regularizers.l1(0.0005))),
