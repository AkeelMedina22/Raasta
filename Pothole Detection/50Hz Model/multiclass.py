...
def train_test_split(data, longitude, latitude):
    ...
    for i in range(0, len(data)-window_size, stride):
        if latitude[i+stride] > 0.1:
            ...
            window_loc.append([latitude[i+stride], longitude[i+stride]])
    ...
    def augment(window, window_loc):
        ...
        for i in range(abs(p)):
            ...
            if np.random.random() < 0.5:
                ...
                new_window.append([new, 'Normal road'])
                new_window_loc.append([normals[index][1][0], normals[index][1][1]])
            else:
        ...
        for i in range(n):
            ...
            new_window.append([potholes[i][0], 'Pothole'])
            new_window_loc.append(potholes[i][1])
        for i in range(n):
...
def evaluate_model(X_train, Y_train, X_test, Y_test):
    ...
    Conv1D(filters=16, kernel_size=30, activation='relu', input_shape=(n_timesteps,n_features), kernel_regularizer=tf.keras.regularizers.L1L2(0.0001)),
    Conv1D(filters=32, kernel_size=15, activation='relu', kernel_regularizer=tf.keras.regularizers.L1L2(0.0001)),
    MaxPooling1D(pool_size=2, strides=2),
    Conv1D(filters=32, kernel_size=10, activation='relu', kernel_regularizer=tf.keras.regularizers.L1L2(0.0001)),
    ...
    tf.keras.models.save_model(model, "Raasta_50hzModel")
