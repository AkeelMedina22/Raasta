    ...
    with open('raasta-c542d-default-rtdb-sensor-data2-export.json', 'r') as f:
        ...
        for key in data:
            for timestamp in data[key]:
                for l in data[key][timestamp]:
                    if l == "accelerometer-x":
                        accelerometer50_x.append(float(data[key][timestamp][l]))

                    elif l == "accelerometer-y":
                        accelerometer50_y.append(float(data[key][timestamp][l]))
           
                    elif l == "accelerometer-z":
                        accelerometer50_z.append(float(data[key][timestamp][l]))
                
                    elif l == "gyroscope-x":
                        gyroscope50_x.append(float(data[key][timestamp][l]))
  
                    elif l == "gyroscope-y":
                        gyroscope50_y.append(float(data[key][timestamp][l]))
  
                    elif l == "gyroscope-z":
                        gyroscope50_z.append(float(data[key][timestamp][l]))
