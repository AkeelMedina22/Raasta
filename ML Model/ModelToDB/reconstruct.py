...
def get_locs():
    ...
    predict = reconstructed_model.predict(X_test)
    ...
    for i in range(len(predict)):
        ...
        if np.argmax(predict[i]) == 2:
            badroads_locations.add(tuple(loc_test_data[i]))
        if np.argmax(predict[i]) == 3:
    ...
    return normalroads_locations, pothole_locations, badroads_locations, speedbreaker_locations
