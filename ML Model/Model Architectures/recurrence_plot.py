...
def train_test_split(data, longitude, latitude):
    ...
    def augment(window):
        ...
        for i in range(len(normals)-len(potholes)):
            ...
            new_fs = int(np.random.uniform(2, 7))
...
def evaluate_model(X_train, Y_train, X_test, Y_test):
    ...
    model = tf.keras.Sequential([Conv2D(filters=8, kernel_size=3, activation='relu', input_shape=(n_features1, n_features2, n_features3)),
    ...
    # fit network
    history = model.fit(X_train, Y_train, validation_data=(X_test, Y_test), epochs=epochs, batch_size=batch_size, verbose=verbose, shuffle=True, callbacks=[tf.keras.callbacks.EarlyStopping(monitor='val_loss', patience=10)])
