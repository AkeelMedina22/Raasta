...
def train_test_split(data):
    ...
    def augment(window):
        ...
        for data in [potholes, normals, speedbreakers]:
            ...
            generator.fit(data, n_fpca_components=120)  
            samples = generator.generate(n_samples=max(500-data.shape[0],0))   
...
def evaluate_model(X_train, Y_train, X_test, Y_test):
    ...
    il = tf.keras.Input(shape=(n_timesteps,n_features))
    l = Conv1D(filters=16, kernel_size=3, activation='relu', input_shape=(n_timesteps,n_features))(il)
    l = Conv1D(filters=32, kernel_size=3, activation='relu')(l)
