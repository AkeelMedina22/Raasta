...
def train_test_split(data, longitude, latitude):
    ...
    def augment(window):
        ...
        for data in [potholes, normals, bads, speedbreakers]:
            ...
            generator.fit(data, n_fpca_components=60)  
            samples = generator.generate(n_samples=400)   
...
def evaluate_model(X_train, Y_train, X_test, Y_test):
    ...
    model.compile(loss=tf.keras.losses.CategoricalCrossentropy(), optimizer=tf.keras.optimizers.Adam(lr=0.0001), metrics=[tf.keras.metrics.CategoricalAccuracy(), tf.keras.metrics.Precision(), tf.keras.metrics.Recall()])
