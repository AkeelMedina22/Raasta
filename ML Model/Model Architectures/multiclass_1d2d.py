...
def evaluate_model(X_train, Y_train, X_test, Y_test):
    ...
    for i in range(X_train.shape[0]):
        ...
        new_X_train.append(X_train[i].T.reshape(6, 60, 1))
    ...
    for i in range(X_test.shape[0]):
        ...
        #     print(X_test[i].T[j].reshape((1,-1)).shape)
        #     image.append(X_test[i].T[j].reshape((1,-1)))
        # new_X_test.append(np.transpose(np.array(image), (1,2,0)))
        new_X_test.append(X_test[i].T.reshape(6, 60, 1))
    ...
    model = tf.keras.Sequential([Conv2D(filters=64, kernel_size=3, activation='relu', input_shape=(n_features1, n_features2, n_features3), kernel_regularizer=tf.keras.regularizers.L1L2(0.0001)),
    Conv2D(filters=32, kernel_size=3, activation='relu', kernel_regularizer=tf.keras.regularizers.L1L2(0.0001)),
    MaxPooling2D(pool_size=(2,2), strides=2),
    # Flatten(),
    tf.keras.layers.Reshape((1, 28*32), input_shape=(1,28,32)),
